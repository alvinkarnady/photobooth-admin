...
export default function CreateFramePage() {
  ...
  const handleSave = async () => {
    ...
    if (!file) return alert('Pilih gambar frame terlebih dahulu!');
    ...
    try {
      ...
      alert('Terjadi kesalahan saat menyimpan frame.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto h-full flex flex-col">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Buat Frame Baru</h1>
          <p className="text-slate-500">Unggah PNG dan atur tata letak slot secara visual.</p>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 flex-1 overflow-hidden">
        ...
        <div className="w-full lg:w-80 bg-white border border-slate-200 rounded-2xl p-6 flex flex-col shadow-sm shrink-0 overflow-y-auto">
          <div className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-2">File Frame (PNG Transparan)</label>
              <input
                type="file"
                accept="image/png"
        ...
        <div className="flex-1 bg-slate-200 rounded-2xl border border-slate-300 overflow-hidden relative flex items-center justify-center p-8 shadow-inner">
          {!previewUrl ? (
            <div className="text-center text-slate-400">
              ...
              <p className="font-medium">Unggah gambar untuk mulai mengedit</p>
            </div>
          ) : (
            <div 
              ref={containerRef}
