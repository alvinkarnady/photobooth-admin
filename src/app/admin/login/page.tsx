...
export default function AdminLogin() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    // Use simple env check, or hardcoded fallback if env is not exposed to client
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 to-sky-50 flex items-center justify-center p-6">
      <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-sm border border-slate-100 text-center animate-fade-in-up">
        ...
        <p className="text-slate-500 mb-6">Masukkan password rahasia</p>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            <input
              ...
              placeholder="••••••••"
              className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent text-center font-bold tracking-widest text-slate-700 bg-slate-50"
            />
          </div>
